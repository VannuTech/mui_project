...
const CourseList1 = () => {
  ...
  const PicturesSet2 = [
    { name: 'BDS', image: picture9 },
    { name: 'MBBS', image: picture10 },
    { name: 'MD/MS', image: picture12 },
  ];
    ...
    >
      ...
      <Grid >
        ...
        {PicturesSet2.map((pic, index) => (
          <Grid item xs={12} sm={12} key={index}>
            <Card sx={{ display: 'flex', flexDirection: 'row', mb: 2, maxWidth: 400, minWidth: 300, maxHeight: 100, alignItems: 'center' }}>
              <CardMedia sx={{ width: 80, height: 60 }} cols={3} rowHeight={4} image={pic.image} alt="pic" />
              <CardContent>
              <Typography variant="h6" fontWeight="bold" fontSize={10}>
                  {pic.name}
